...
if __name__ == "__main__":
    ...
    GNN_model = GraphSAGE(in_channels = 2, hidden_channels=32, out_channels=1, num_layers=2, aggr='max')
    print(summary(GNN_model, x = td["graph"]["x"][0], edge_index = td["graph"]["edge_index"][0]))
    GNN_Actor = GNNTensorDictModule(GNN_model, in_keys=["graph"], out_keys=['logits'])  # create actor
    #td = GNN_Actor(td) # forward pass



    GNN_Module = train_GNN_module(GNN_Actor, td, in_keys = ["graph"], num_training_epochs = 100, lr = 0.001,)
