...
class SingleHop(EnvBase):
    ...
    def _extract_X_gen(self, X_params):
        ...
        X_gen = [create_rv(self.np_rng, nums = 0, probs = 1)]
        self.arrival_rates = []
        for key, value in X_params.items():
            X_gen.append(create_rv(self.np_rng, nums = value["arrival"], probs = value['probability']))
            X_map[key] = {"source": value['source'], "destination": value['destination']}
            self.arrival_rates.append(value['probability'])
        self.X_map = X_map
        self.X_gen = X_gen
    ...
    def _extract_Y_gen(self, Y_params):
        ...
        Y_gen = [create_rv(self.np_rng, nums = 1, probs = 1)]
        link_rates = []
        for key, value in Y_params.items():
            Y_gen.append(create_rv(self.np_rng, nums = value['capacity'], probs = value['probability']))
    ...
    def _compute_load(self):
        ...
        self.network_load = np.mean([x/y for x,y in zip(self.arrival_rates, self.link_rates)])
    ...
    def _step(self, tensordict: TensorDict):
        ...
        if self.terminal_backlog is not None:
            done = self.get_backlog() > self.terminal_backlog
