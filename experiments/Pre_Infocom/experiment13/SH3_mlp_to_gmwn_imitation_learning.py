...
if __name__ == "__main__":
    ...
    # Create MaxWeight NN
    gmwn_agent = create_gnn_maxweight_actor_critic(input_shape=input_shape,
    ...
    train_module2(gmwn_agent, replay_buffer, num_training_epochs=training_epochs, lr=learning_rate,
    ...
    # w = gmwn_agent.mwn_weights().detach().squeeze().numpy()[1:]
    ...
    with torch.no_grad(), set_exploration_type(ExplorationType.MODE):
        ...
        for n in range(num_rollouts):
            ...
            env = env_generator.sample(env_id)
            mw_nn_td = env.rollout(policy=gmwn_agent, max_steps=rollout_length)
    ...
    # Create a new MLP agent
    mlp_agent2 = create_actor_critic(
        input_shape,
        output_shape,
        in_keys=["observation"],
        action_spec=base_env.action_spec,
    ...
    # Train the MLP agent on the same data
    train_module2(mlp_agent2, replay_buffer, num_training_epochs=training_epochs, lr=learning_rate,
                    loss_fn=nn.CrossEntropyLoss(), weight_decay=0)

    # test the new mlp agent
    with torch.no_grad(), set_exploration_type(ExplorationType.MODE):
        mlp_tds2 = []
        env_generator.reseed(env_generator_seed)
        for n in range(num_rollouts):
            env = env_generator.sample(env_id)
            print(f"Collecting trajectory {n} from MLP agent")
            mlp_td = env.rollout(policy=mlp_agent2, max_steps=rollout_length)
            mlp_tds2.append(mlp_td)

    mlp_ltas2 = [compute_lta(td["backlog"]) for td in mlp_tds2]
    mlp_mean_lta2 = np.mean(mlp_ltas2, axis=0)
    mlp_std_lta2 = np.std(mlp_ltas2, axis=0)
    ...
    ax[1].plot(mlp_mean_lta2, label="Imitation MLP Agent")
    ax[1].fill_between(range(len(mlp_mean_lta2)), mlp_mean_lta2 - mlp_std_lta2, mlp_mean_lta2 + mlp_std_lta2, alpha=0.2)
    ...
    ax[2].plot(mlp_mean_lta2 / mw_mean_lta, label="Imitation MLP Agent")
    ...
    ax[2].text(len(mlp_mean_lta2) + 200, mlp_mean_lta2[-1] / mw_mean_lta, f"{mlp_mean_lta2[-1] / mw_mean_lta:.4f}")
...
headers = ["Q", "Y", "MWN Action", "MLP Action"]
...
for state, (mlp_action, mwn_action) in diffs.items():
    ...
    table_data.append([Q, Y, mwn_action[1:], mlp_action[1:]])
