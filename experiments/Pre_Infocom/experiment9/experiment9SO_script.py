...
parser.add_argument('--experiment_name', type=str, help='what the experiment will be titled for wandb', default="Experiment9_SO")
...
train_sets = {"a": {"train": [22], "test": []}, # lta backlog = 135.10
              ...
              "c": {"train": [24], "test": []},} # 12.048


SCRIPT_PATH = os.path.dirname(os.path.abspath(__file__))
CONFIG_PATH = os.path.join(SCRIPT_PATH, "experiment9SO.yaml")
...
critic.parameters(),
lr=cfg.critic_optim.lr,
weight_decay=cfg.critic_optim.weight_decay,
eps=cfg.critic_optim.eps,
)

# actor_optim = torch.optim.LBFGS(
#     actor.parameters(),
#     lr = cfg.actor_optim.lr,
# )

actor_optim = Minimizer(
...
for i, data in enumerate(collector):
    ...
    for j in range(cfg.loss.ppo_epochs):
        ...
        for k, batch in enumerate(data_buffer):
            ...
            alpha = 1.0
            if cfg.critic_optim.anneal_lr:
                alpha = 1 - (num_network_updates / total_network_updates)
                for group in critic_optim.param_groups:
                    group["lr"] = cfg.critic_optim.lr * alpha
    ...
    log_info.update(
        {
            "train/lr": alpha*cfg.critic_optim.lr,
