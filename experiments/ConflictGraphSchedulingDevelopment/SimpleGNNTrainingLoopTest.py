...
class GNN_Critic(TensorDictModule):
    ...
    def __init__(self, module, in_key = "batch_graph", out_key="state_value"):
        super(GNN_Critic, self).__init__(module=module, in_keys=["batch_graph"], out_keys=[out_key])
        self.in_key = in_key
        self.out_key = out_key

    def forward(self, input):
        if isinstance(input, TensorDict):
            input[self.out_key] = self.module(
                input[self.in_key].x, input[self.in_key].edge_index, input[self.in_key].batch).squeeze(-1)
            return input
...
actor = GNN_Actor(module = policy_module, x_key = "observation", edge_index_key = "adj_sparse", out_key = "probs")
...
critic = GNN_Actor(module = value_module, x_key="observation", edge_index_key="adj_sparse", out_key="state_value")
