...
def create_state_action_map(td, action_key = "action"):
    state_action_map = {}
    for Q, Y, action in zip(td["Q"], td["Y"], td[action_key]):
        state = tuple(Q.tolist()+ Y.tolist())
        if action_key == "action":
            action = action.argmax().item()
        elif action_key == "logits":
            action = action.softmax(dim = 0).round(decimals=2)
...
for i in range(num_rollouts):
    state_action_map = create_state_action_map(results["MLP"][i]["td"])
    if i == 0:
...
for i in range(num_rollouts):
    state_logits_map = create_state_action_map(results["MLP"][i]["td"], "logits")
    if i == 0:
