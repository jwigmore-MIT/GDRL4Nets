...
mlp = MLP(in_features = input_shape[0], out_features = output_shape, activation_class= torch.nn.ReLU, depth = 2, num_cells = 64)
...
logger = get_logger(
        "wandb",
        logger_name="..\\logs",
        experiment_name= f"DQN_SH1E_{time.strftime('%%m_%d-%H_%M_%S')}",
        wandb_kwargs={
            "project": "Experiment15",
        },
    )
...
for i, data in enumerate(collector):
    ...
    with torch.no_grad(), set_exploration_type(ExplorationType.MODE):
        ...
        if (i >= 1 and (prev_test_frame < cur_test_frame)) or final:
            q_module.eval()
            eval_start = time.time()
            eval_log_info, eval_tds = evaluate_dqn_agent(q_module, eval_env_generator, [training_env_generator.history[-1]], pbar, cfg, device=device)
...
with torch.no_grad(), set_exploration_type(ExplorationType.MODE):
    ...
    q_module.eval()
    eval_start = time.time()
    eval_log_info = evaluate_dqn_agent(q_module, eval_env_generator, [training_env_generator.history[-1]], pbar, cfg, device=device)
