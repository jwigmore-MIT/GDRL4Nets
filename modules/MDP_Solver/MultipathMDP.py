...
class MultipathMDP(MDP):

    def __init__(self, env, name = "MultipathMDP", q_max = 10, discount = 0.99, value_iterator = "normal"):
    ...
    def _get_state_list(self, env, q_max):
        ...
        STATES ARE THE SAME AS IN SINGLEHOP
        """
        ...
        high_links = np.array([rv.max() for rv in env.Y_gen])
...
def form_transition_matrix2(env, state_list, action_list, q_max = 30):
    ...
    X=1
    all_y_dist = [Y.dist() for Y in env.Y_gen]
