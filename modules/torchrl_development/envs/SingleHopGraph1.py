...
from modules.torchrl_development.envs.utils import TimeAverageStatsCalculator, create_discrete_rv, create_poisson_rv, FakeRV
...
class SingleHopGraph(EnvBase):
    ...
    def __init__(self, net_para, seed = None, device = "cpu"):
        ...
        # If we want to generate x matrix for outputs
        self.generate_x_output = net_para.get("generate_x_output", False)
    ...
    def _make_spec(self):
        ...
        if self.generate_x_output:
            self.observation_spec.set("x", UnboundedContinuousTensorSpec(
                shape=(len(self.nodes) - 1, 2),
                # might want to change the dimensions to add the context as the observation
                dtype=torch.float
            ))
        if self.track_stdev:
    ...
    def _step(self, tensordict: TensorDict):
        ...
        if self.generate_x_output:
            out.set("x", torch.tensor(np.stack([self.get_Q_out(), self.get_Y_out()], axis = 1), dtype = torch.float))
        if self.obs_lambda:
